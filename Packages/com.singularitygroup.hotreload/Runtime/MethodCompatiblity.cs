...
namespace SingularityGroup.HotReload {
    static class MethodCompatiblity {
        ...
        static bool LikelyHasExplicitThis(ParameterInfo[] prevParams, ParameterInfo[] patchParams, MethodBase previousMethod) {
            return patchParams.Length == prevParams.Length + 1 
                   && patchParams[0].ParameterType == previousMethod.DeclaringType
                   && (prevParams.Length < 1 || prevParams[0].ParameterType != previousMethod.DeclaringType)
                   && patchParams[0].Name == "this";
        }
