...
namespace ThunderFireUITool
{
    public class PrefabDetailWindow : EditorWindow
    {
        ...
        public static void OpenWindow(PrefabDetail prefabDetail)
        {
            int width = 400;
            int height = 270;
            _managementLevels = HierarchyManagementWindow.GetManagementLevels();
            _prefabDetails = new List<PrefabDetail>();
            foreach (var item in HierarchyManagementWindow.GetPrefabDetails())
            {
            ...
            _mLevel = _managementLevels[prefabDetail.LevelID];
        ...
        private void InitWindowData()
        {
            ...
            foreach (var level in _managementLevels)
            {
                _mOptions.Add(level.ChannelID * _range +
                              _managementChannels[level.ChannelID].LevelIDList.IndexOf(level.ID) + "");
            }
        }
        ...
        private void DrawWindowUI()
        {
            ...
            UXBuilder.Select(col, new UXBuilderSelectStruct()
            {
                ...
                defaultValue = _mLevel != null
                    ? _mLevel.ChannelID * _range +
                      _managementChannels[_mLevel.ChannelID].LevelIDList.IndexOf(_mLevel.ID) + ""
                    : "",
                onChange = s =>
                {
                    foreach (var level in _managementLevels)
                    {
                        if (level.ChannelID * _range +
                            _managementChannels[level.ChannelID].LevelIDList.IndexOf(level.ID) + "" == s)
                        {
                            _mLevel = level;
                        }
                    }
        ...
        private void Submit()
        {
            ...
            if (_prefabDetails.Find(s => s.Guid == _mGuid) != null)
            {
            ...
            {
                _prefabDetails[_mPrefabSubmit.ID] = _mPrefabSubmit;
                if (_mPrefabSubmit.LevelID != _mLevel.ID)
                {
                    _managementLevels[_mPrefabSubmit.LevelID].PrefabDetailIDList.Remove(_mPrefabSubmit.ID);
                    _mLevel.PrefabDetailIDList.Insert(0, _mPrefabSubmit.ID);
                    _mPrefabSubmit.Guid = _mGuid;
                    _mPrefabSubmit.LevelID = _mLevel.ID;
                    _mPrefabSubmit.ChannelID = _mLevel.ChannelID;
            ...
            HierarchyManagementWindow.SetMaxPrefabNum(maxPrefabNum);

            HierarchyManagementWindow.SetPrefabDetails(_prefabDetails);
        ...
        private int DealWithTag(TagDetail tag)
        {
            ...
            foreach (var s in _prefabDetails)
            {
                if (s.Tags.Contains(tag) && s.ID != _mPrefabSubmit.ID && s.LevelID <= _mLevel.ID)
                {
            ...
            if (prefab != null)
            {
                if (prefab.LevelID < _mLevel.ID)
                {
                    ...
                    foreach (var item in _prefabDetails.FindAll(s =>
        ...
        private TagDetail AddTag(VisualElement tagContent, string str)
        {
            ...
            var prefab = HierarchyManagementWindow.GetTagColors().Find(s => s.Name == tag.Name);
            if (prefab == null)
            {
                HierarchyManagementWindow.GetTagColors().Add(new TagColor()
                {
                    Name = tag.Name,
                    Color = Color.black,
                });
        ...
        {
            //判断是否要进行tag的减
            var flag = _prefabDetails.FindAll(s => s.Tags.Contains(tag) && s.ID != _mPrefabSubmit.ID)
