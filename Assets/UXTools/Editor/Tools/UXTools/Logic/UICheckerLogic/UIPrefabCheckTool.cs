...
{
    public class UIPrefabCheckTool : OdinEditorWindow
    {
        private string prefabName;

        [HorizontalGroup("Data Group", 0.5f)]
        [BoxGroup("Data Group/ ")]
        [ReadOnly, LabelText("当前Prefab中Atlas信息")]
        public List<AtlasData> atlasDatas = new List<AtlasData>();

        [BoxGroup("Data Group/  ")]
        [TF_TableList(AlwaysExpanded = true)]
        [ReadOnly, LabelText("当前Prefab中Image信息")]
        public List<SingleImageRefData> imageDatas = new List<SingleImageRefData>();

        [BoxGroup("Data Group/  ")]
        [TF_TableList(AlwaysExpanded = true)]
        [ReadOnly, LabelText("@GetTextCheckResult()")]
        public List<NodeFontData> nodeFontDatas = new List<NodeFontData>();


        [BoxGroup("Data Group/  ")]
        [TF_TableList(AlwaysExpanded = true)]
        [ReadOnly, LabelText("@GetComponentCheckResult()")]
        ...
        public static UIPrefabCheckTool CheckAtlasButton()
        {
            UIPrefabCheckTool window = (UIPrefabCheckTool)EditorWindow.GetWindow(typeof(UIPrefabCheckTool));
        ...
        protected override void OnGUI()
        {
            ...
            if (checkAtlasSettings != null)
            {
                if (imageDatas.Count > checkAtlasSettings.imageLimit)
                    imageCountString = imageCountString + redColorFlag + imageDatas.Count + ColorFlagEnd;
                else
                    imageCountString = imageCountString + greenColorFlag + imageDatas.Count + ColorFlagEnd;
            }
            else
            {
                imageCountString = imageCountString + imageDatas.Count;
            }
        ...
        public void CheckPrefabUI(GameObject go = null)
        {
            ...
            imageDatas = imageDatas.OrderByDescending(p => p.resolution).ToList();
        ...
        public void CheckAtlas(GameObject root)
        {
            ...
            for (int i = 0; i < imageList.Length; i++)
            {
                ...
                var texture = GetRuntimeTexture(imageList[i].sprite);

                bool isAtlas = texture is SpriteAtlas;
                if (isAtlas)
                {
                    //一个图集中的图素
                    int textureIndex = FindTextureInAtlasList(texture);
                    if (textureIndex == -1)
                    {
                        SpriteAtlas atlas = texture as SpriteAtlas;
                        SpriteAltasFillingInfo AltasFillingInfo = UIResCheckTool.GetAltasFillingInfo(atlas);
                ...
                {
                    //一个散图
                    int textureIndex = FindTextureInImageList(texture);
                    if (textureIndex == -1)
                    {
                        Texture2D tex = texture as Texture2D;
                        bool inRule = UIResCheckTool.GetImageIsInRule(tex);
                        AtlasResolutionItem atlasResolution = new AtlasResolutionItem();
                        atlasResolution.Add(tex.width, tex.height);
                        imageDatas.Add(new SingleImageRefData()
                        {
                            image = imageList[i],
        ...
        private UnityEngine.Object GetRuntimeTexture(Sprite sprite)
        {
            ...
            if (folderName.EndsWith("_atlas"))
            {
                SpriteAtlas atlas = AssetDatabase.LoadAssetAtPath<SpriteAtlas>($"Assets/Res/GUI/Atlas/{folderName}.spriteatlas");
                if (atlas != null)
                {
        ...
        {
            Font defaultFont = AssetDatabase.LoadAssetAtPath<Font>("Assets/Res/GUI/Fonts/Default.otf");
        ...
        private void CheckNodeDeprecate(GameObject go)
        {
            ...
            foreach (Transform child in children)
            {
                ...
                foreach (Component component in components)
                {
                    ...
                    PrefabYamlUtils.LoadFromString(prefabYaml, ref assignedInstanceIdList);
                }
            }
