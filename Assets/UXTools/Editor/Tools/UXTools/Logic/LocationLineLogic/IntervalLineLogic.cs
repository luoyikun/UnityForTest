...
namespace ThunderFireUITool
{
    ...
    public class IntervalLineLogic : UXSingleton<IntervalLineLogic>
    {
        ...
        private void ResetAll()
        {
            ...
            {
                if (EditorLogic.ObjectFit(item.gameObject) && item.gameObject != m_SelectedObject)
                {
    ...
    public class VisualLineManager
    {
        ...
        private void DrawLines()
        {
            ...
            if (m_VertLines != null)
            {
                float offset = sceneView.rootVisualElement.style.top.value.value + sceneView.rootVisualElement.style.bottom.value.value + 27;
                foreach (var line in m_VertLines)
                {
                    ...
                    line.visual.style.top = sceneView.camera.pixelHeight - vec2.y + offset;
                    if (line.needNum)
                    {
                        Label label = line.visual.Q<Label>("lengthLabel");
