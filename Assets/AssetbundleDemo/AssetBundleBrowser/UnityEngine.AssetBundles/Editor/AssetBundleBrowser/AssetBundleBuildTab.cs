...
namespace UnityEngine.AssetBundles
{
    ...
    public class AssetBundleBuildTab
    {
        ...
        private void ExecuteBuild()
        {
            ...
            //opt = BuildAssetBundleOptions.IgnoreTypeTreeChanges | BuildAssetBundleOptions.DeterministicAssetBundle; 
            ...
            string androidMain = m_OutputPath + "/Android.assetbundle";
            if (File.Exists(androidMain))
            {
                File.Delete(androidMain);
            }

            string iOSMain = m_OutputPath + "/iOS.assetbundle";
            if (File.Exists(iOSMain))
            {
                File.Delete(iOSMain);
            }
            paths.Clear(); files.Clear();
            Recursive(m_OutputPath);
            ...
            UnityEngine.Debug.Log("Build Assetbundle OK !!!!!!!!");
        }
         ...
         void Recursive(string path)
        {
            ...
            foreach (string filename in names)
            {
                ...
                //string fileName = @"c:/1.jpg"; string dfileName = System.IO.Path.ChangeExtension(fileName, ".png"); File.Move(fileName, dfileName);

                if (filename == m_OutputPath + "\\Android")
                {
                    string dfileName = System.IO.Path.ChangeExtension(filename, ".assetbundle");
                    if (File.Exists(dfileName))
                    {
                        File.Delete(dfileName);
                    }
                    File.Move(filename, dfileName);
                    files.Add(dfileName.Replace('\\', '/'));
                }
                else if (filename == m_OutputPath + "/iOS")
                {
                    string dfileName = System.IO.Path.ChangeExtension(filename, ".assetbundle");
                    if (File.Exists(dfileName))
                    {
